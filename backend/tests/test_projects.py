...
def test_admin_can_rename_project(client, db_session):
...
def test_gestor_cannot_delete_project(client, db_session):
    _create_and_login(client, db_session, "admin13", "senha1234", UserRole.ADMIN)
    project_id = client.post("/api/projects", json={"name": "Projeto Protegido"}).json()["id"]
